...
function [Simulation,dX,P,alpha,beta] = eskf_update_VB_IW(Simulation,dX_,P_,dz,H,M,I,alpha_,beta_)
...
%         beta1 = H1*(0.5*beta_ + 0.5*0.5*H(:,1:9)'*(v.^2 + diag(H*P_VB*H')))/i;
         beta1 = H1*(beta_ + H(:,1:9)'*(v.^2 + diag(H*P_VB*H')));
    end
    beta=beta1+beta2;
