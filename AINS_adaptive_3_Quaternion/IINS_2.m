...
include_compass          = Simulation.Auxiliary_Snsr.include_compass;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fs_GPS=1/mean(diff(Simulation.Input.Measurements.GPS(:,1)));
fs_Depth=1/mean(diff(Simulation.Input.Measurements.Depth(:,1)));
fs_DVL=1/mean(diff(Simulation.Input.Measurements.DVL(:,1)));
fs_heading=1/mean(diff(Simulation.Input.Measurements.Compass(:,1)));
        fs_auxiliarySensor={fs_GPS,fs_Depth,fs_DVL,fs_heading};
       ...
       %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
                ...
                tic
                    ...
                    Selection_Param1 ={include_GPS,include_depthmeter,include_dvl,include_compass,include_accelrollpitch ,muu };
%                     format shortG;
                        t_imu = zeros(Dlength,1);t_imu(1,1)=0;
%                         t_GPS = zeros(length(Simulation.Input.Measurements.GPS),1);t_GPS(1,1)= Simulation.Input.Measurements.GPS(1,1);
                  I=ave_sample+1;
                   while I <= Dlength
...
%                         end
                        if Simulation.Input.Measurements.hdng_Counter < length(Simulation.Input.Measurements.Compass)
                            hdng_Time  = num2str((round(Simulation.Input.Measurements.compass(Simulation.Input.Measurements.hdng_Counter,1)*1000))/1000);
%                             hdng_Time  = Simulation.Input.Measurements.Compass(Simulation.Input.Measurements.hdng_Counter,1);
                        end 
                        ...
                        [Simulation,fb,Wib_b]=SINS_2(Simulation , I , dt ,func_f, fc_f_RP , fc_f_accel , fc_f_gyro, filtered_accel , filtered_gyro ,ave_sample,Selection_Param{11},coeff_final,include_MA);                 
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                               
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        [Simulation,flag_Qadapt] = ESKF(Simulation,I,Selection_Param,C_DVL_IMU,Include_R_adaptive,Include_Q_adaptive,Include_VB_adaptive, Include_IF_VB_adaptive,flag_Qadapt,Misalignment_IMU_phins,ave_sample,calib_sample,SF,dt,fs_auxiliarySensor);
                        [Simulation] = State_Correction( Simulation , I, ave_sample );                      
                        x = [Simulation.Output.INS.X_INS(I-ave_sample,1:3),Simulation.Output.INS.X_INS(I-ave_sample,4:6),Simulation.Output.INS.X_INS(I-ave_sample,7:10)]; 
                        C = Simulation.Output.INS.Cbn(:,:,I-ave_sample);
                        [Simulation,flag_Qadapt]=AQ_calcul(x,C,Simulation,fs,I,Include_Q_adaptive,flag_Qadapt,fb,Wib_b);  
                    
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        
                         if I== calib_sample
% %                             [ Simulation ] = R_Moving( Simulation );
                             [  Simulation , include_GPS , include_depthmeter , include_dvl , include_compass , include_accelrollpitch ] = Moving_Selection(Simulation);
%                           
                             Selection_Param1 = {include_GPS , include_depthmeter , include_dvl , include_compass , include_accelrollpitch , muu };
%                    
                         end
...
%                 end         
%                 %&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
%                    [Simulation]= creat_RefPos(Simulation,t_imu);
                   %&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
