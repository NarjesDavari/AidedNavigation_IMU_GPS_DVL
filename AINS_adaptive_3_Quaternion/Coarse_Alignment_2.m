function [ Simulation , gl, ave_fb , ave_W ] = Coarse_Alignment_2( Simulation , ave_sample )    
...
%     Alt = zeros(ave_sample,1);
    ...
    Simulation.Output.INS.X_INS      = zeros(Dlength-ave_sample+1 ,16);
    Simulation.Output.INS.X_INS_BEuler = zeros(Dlength-ave_sample+1 ,15);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     end
    ...
    if ~isempty(nonzeros(CA.Roll))
        Simulation.Output.INS.X_INS_BEuler(1,7) = mean (CA.Roll);
    else
       Simulation.Output.INS.X_INS_BEuler(1,7) = 0;
    end
    if ~isempty(nonzeros(CA.Pitch))
        Simulation.Output.INS.X_INS_BEuler(1,8) = mean (CA.Pitch);
    else
        Simulation.Output.INS.X_INS_BEuler(1,8) = 0;
    end
    if ~isempty(nonzeros(CA.Yaw))
        Simulation.Output.INS.X_INS_BEuler(1,9) = mean (nonzeros(CA.Yaw)); %%+ Simulation.Parameters_Misalignment.IMU_phins(3)*pi/180;
    else
        Simulation.Output.INS.X_INS_BEuler(1,9) = 0;
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   
Euler1=Simulation.Output.INS.X_INS_BEuler(1,7:9);
Simulation.Output.INS.X_INS(1,7:10) = convert_Euler2Quaternion (Euler1);
Cbn   = InCBN(Euler1);
V1n=(Cbn*V1b')';
Simulation.Output.INS.X_INS(1,4:6) =V1n;
 Simulation.Output.INS.X_INS_BEuler(1,1:6)=Simulation.Output.INS.X_INS(1,1:6);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %IMU
