...
%     Simulation.Output.EKF.X      = zeros(Dlength-ave_sample+1 ,15);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     end
    ...
    Simulation.Output.EKF.X(1,1) = mean (nonzeros(Lat));
    Simulation.Output.EKF.X(1,2) = mean (nonzeros(lon));
    if ~isempty(nonzeros(Z))
        Simulation.Output.EKF.X(1,3) = mean (nonzeros(Z));
    else
        Simulation.Output.EKF.X(1,3) = 0;
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    gl = Gravity( [Simulation.Output.EKF.X(1,1), Simulation.Output.EKF.X(1,2),Simulation.Output.EKF.X(1,3)] );
    
    CA.Pitch = asin((fb(:,1))./gl(3));
    CA.Roll  = -asin((fb(:,2))./(gl(3).*cos(CA.Pitch)));    
    %Initial roll, pitch, heading
    if ~isempty(nonzeros(CA.Roll))
        Simulation.Output.EKF.X(1,7) = mean (CA.Roll);
    else
        Simulation.Output.EKF.X(1,7) = 0;
    end
    if ~isempty(nonzeros(CA.Pitch))
        Simulation.Output.EKF.X(1,8) = mean (CA.Pitch);
    else
        Simulation.Output.EKF.X(1,8) = 0;
    end
    if ~isempty(nonzeros(CA.Yaw))
        Simulation.Output.EKF.X(1,9) = mean (nonzeros(CA.Yaw)); %%+ Simulation.Parameters_Misalignment.IMU_phins(3)*pi/180;
    else
        Simulation.Output.EKF.X(1,9) = 0;
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Euler1=Simulation.Output.EKF.X(1,7:9);
Cbn   = InCBN(Euler1);
V1n=(Cbn*V1b')';
Simulation.Output.EKF.X(1,4:6) =V1n;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %IMU
