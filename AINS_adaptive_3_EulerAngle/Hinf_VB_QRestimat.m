...
function [Simulation,dX,P,u_hat,U_hat_f,P_hat,t_hat]= Hinf_VB_QRestimat(Simulation,dX_,P_, dz ,H,M , I, update_counter,gama2)
...
% % P = (eye(15)-K*H)*P*(eye(15)-K*H)' + K*R*K';
     %%%%%%%%%%%%%%%%%%%%%%%%%%%Hinf
   R2_hat =H(:,1:9)*M*R_hat*M'*H(:,1:9)';
   eta = (eye(length(dX)) - gama2 * P_hat + H'*R2_hat^(-1)* H*P_hat);
%    [u,d,v1]=svd(eta1);
%         eta=u*sqrt(d)*v1';
        K = P_hat*eta^(-1)*H'*R2_hat^(-1);    %Computed Kalman gain 
        dX = dX_ + K*(dz- H*dX_);
         P = P_hat * eta^(-1);
%       P = P_hat - K * H* P_hat;
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
